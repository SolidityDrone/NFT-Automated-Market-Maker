...
contract YakiSwap is ReentrancyGuard {
    event Pool_Creation(
    ...
    mapping(uint256 => address) internal s_Pool_Address_By_ID;
    mapping(address => uint256) internal s_Pool_ID_By_Address;
...
contract Pool is IERC721Receiver, ReentrancyGuard {
    ...
    function swapNFT(uint256 tokenIn, uint256 tokenOut) public payable {
        ...
        _parent.handlePoolEvent(msg.sender, getSwapFee(), tokens, YakiSwap.EventType.USER_SWAP);
    }
    ...
    function batchSellNFT(
        ...
        _parent.handlePoolEvent(msg.sender, msg.value, tokenId, YakiSwap.EventType.USER_BUY);
    ...
    function batchBuyNFT(uint256[] calldata tokenId) public nonReentrant {
        ...
        _parent.handlePoolEvent(msg.sender, cumulativePrice, tokenId, YakiSwap.EventType.USER_SELL);
    ...
    function _absoluteModule(int256 x) internal pure returns (uint256) {
